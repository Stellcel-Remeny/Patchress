void get_fancy_names(char **all_items, char **menus, char **entries, const char *current_directory) {
    ...
    while (menus[i] != NULL) {
        ...
        if (all_items[i]) {
            strncpy(all_items[i], fancy_name, strlen(fancy_name) + 1);
        } else {
            all_items[i] = malloc(strlen(menus[i]) + 1);
            if (all_items[i]) {
                strncpy(all_items[i], menus[i], strlen(menus[i]) + 1);
            }
        }
        i++;
    }
    ...
    while (entries[i] != NULL) {
        ...
        if (all_items[menu_count + i]) {
            strncpy(all_items[menu_count + i], fancy_name, strlen(fancy_name) + 1);
        } else {
            all_items[menu_count + i] = malloc(strlen(entries[i]) + 1);
            if (all_items[menu_count + i]) {
                strncpy(all_items[menu_count + i], entries[i], strlen(entries[i]) + 1);
            }
        }
        i++;
    }
...
// TODO: Add a way to edit execution arguments before running.
    ...
    // Init variables
    static char current_directory[PATH_MAX] = {0},
                init_dir[PATH_MAX] = {0},
    ...
    while (true) {
        ...
        if (entry_runs_on_msdos && entry->exe[0] != '\0') {
            if (key == 13) { // ENTER key
                ...
                sprintf(cmd, "%s %s", entry->exe, entry->args);
